...
<div class="max-w-5xl mx-auto mt-8 bg-white shadow-xl rounded-xl overflow-hidden border border-slate-200">
    ...
    {{-- Main --}}
    <main class="p-12">
        <h2 class="text-2xl font-bold text-center text-slate-800 mb-2">Eliminar proveedor</h2>
        ...
        <div class="max-w-2xl mx-auto space-y-6 mb-10">
            ...
            <div class="flex flex-col">
                <label class="text-sm font-semibold text-slate-500 mb-1">ID del proveedor:</label>
                <input type="text" value="{{ $proveedor->id }}" disabled
                       class="input-line w-full bg-transparent text-lg text-slate-400 cursor-not-allowed">
            </div>

            <div class="flex flex-col">
                <label class="text-sm font-semibold text-slate-500 mb-1">Nombre:</label>
                <input type="text" value="{{ $proveedor->nombre }}" disabled
                       class="input-line w-full bg-transparent text-lg text-slate-400 cursor-not-allowed">
            </div>

            <div class="flex flex-col">
                <label class="text-sm font-semibold text-slate-500 mb-1">Contacto:</label>
                <input type="text" value="{{ $proveedor->contacto }}" disabled
                       class="input-line w-full bg-transparent text-lg text-slate-400 cursor-not-allowed">
            </div>
            ...
            <div class="flex flex-col">
                <label class="text-sm font-semibold text-slate-500 mb-1">Email:</label>
            ...
            <div class="flex flex-col">
                ...
                <textarea rows="3" disabled
                          class="input-line w-full bg-transparent text-lg text-slate-400 cursor-not-allowed">{{ $proveedor->direccion }}</textarea>
            </div>
        ...
        <div class="max-w-2xl mx-auto bg-red-50 border border-red-200 rounded-xl p-6 mb-8 flex items-start gap-4">
            ...
            <div>
                ...
                <p class="text-red-500 text-sm">Vas a eliminar este proveedor. Esta acción no se puede deshacer.</p>
            </div>
        </div>

        {{-- Botones --}}
        <div class="flex justify-center space-x-6">
            <form action="{{ route('admin.proveedors.destroy', $proveedor->id) }}" method="POST">
                @csrf
                @method('DELETE')
                <button type="submit"
