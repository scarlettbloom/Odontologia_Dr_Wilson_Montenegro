...
<div class="card-container">
    ...
    <main class="p-12">
        ...
        <form action="{{ route('admin.inventario.update', $item->idinventario) }}" method="POST" class="max-w-2xl mx-auto space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="flex flex-col">
                <label class="text-sm font-semibold text-slate-500 mb-1">Proveedor:</label>
                <select name="nombre_proveedor" id="nombre_proveedor" class="select-proveedor">
                    @foreach($proveedors as $proveedor)
                        <option value="{{ $proveedor->nombre }}" {{ $item->nombre_proveedor == $proveedor->nombre ? 'selected' : '' }}>
                            {{ $proveedor->nombre }}
                        </option>
                    @endforeach
                </select>
